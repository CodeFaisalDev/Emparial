  ...
  return (
    <PageTransition>
      <div className="px-4 pt-32 pb-16 flex flex-col gap-6 max-w-[1600px] mx-auto">
        ...
        {/* Header Section */}
        <section className="bg-white rounded-[2.5rem] p-12 lg:p-20 flex flex-col lg:flex-row gap-12 items-center justify-between">
          <div className="lg:w-1/2">
            <h1 className="text-[3.5rem] md:text-[5rem] font-bold tracking-tighter leading-[1.05] mb-6">
          ...
          <div className="lg:w-1/2 w-full h-[300px] lg:h-[400px] relative rounded-[2rem] overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          ...
          <div className="lg:col-span-4">
            <div className="bg-navy rounded-[2.5rem] p-10 text-white flex flex-col justify-between min-h-[400px] sticky top-32">
               <div>
                  <h2 className="text-3xl font-bold mb-4">Still have questions?</h2>
                  <p className="text-white/70 mb-8 leading-relaxed">Our support team is ready to help you with your specific architectural needs. Let's discuss your vision today.</p>
               </div>
          ...
          <div className="lg:col-span-8 bg-white rounded-[2.5rem] p-10 lg:p-16">
            {faqs.map((group, groupIdx) => (
              <div key={group.category} className="mb-12 last:mb-0">
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-8 h-[2px] bg-navy/20"></div>
                  <h3 className="text-xl font-bold text-navy/50 uppercase tracking-widest">{group.category}</h3>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  {group.items.map((faq, itemIdx) => {
                    ...
                    return (
                      <div key={id} className="border border-navy/10 rounded-[1.5rem] overflow-hidden transition-all duration-300 hover:shadow-[0_10px_30px_rgba(0,0,0,0.05)] hover:border-navy/20">
                        <button 
                          onClick={() => setOpenIndex(isOpen ? null : id)}
                          className="w-full flex justify-between items-center p-6 lg:p-8 text-left bg-transparent group"
                        >
                          <span className="text-xl lg:text-2xl font-semibold pr-8 group-hover:text-navy transition-colors">{faq.q}</span>
                          <span className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center transition-colors duration-500 ${isOpen ? 'bg-navy text-white' : 'bg-[#f8f9fa] text-navy'}`}>
                        ...
                        <AnimatePresence>
                          {isOpen && (
                            ...
                            >
                              <div className="px-6 lg:px-8 pb-8 text-navy/70 leading-relaxed text-lg border-t border-navy/5 pt-6 mx-4">
                                {faq.a}
                              </div>
                            </motion.div>
                          )}
